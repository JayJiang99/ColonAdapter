...
class Attention(nn.Module):
    ...
    def add_lora(self, rank=4, alpha=1.0, dropout=0.0):
        """Convert attention linear layers to LoRA versions"""
        from dust3r.lora import add_lora_to_linear_layer
        self.qkv = add_lora_to_linear_layer(self.qkv, rank, alpha, dropout)
        self.proj = add_lora_to_linear_layer(self.proj, rank, alpha, dropout)
...
class CrossAttention(nn.Module):
    ...
    def add_lora(self, rank=4, alpha=1.0, dropout=0.0):
        """Convert attention linear layers to LoRA versions"""
        from dust3r.lora import add_lora_to_linear_layer
        self.projq = add_lora_to_linear_layer(self.projq, rank, alpha, dropout)
        self.projk = add_lora_to_linear_layer(self.projk, rank, alpha, dropout)
        self.projv = add_lora_to_linear_layer(self.projv, rank, alpha, dropout)
        self.proj = add_lora_to_linear_layer(self.proj, rank, alpha, dropout)    
