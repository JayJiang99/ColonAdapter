...
class PoseHead(nn.Module):
    ...
    def __init__(self, 
        ...
        # Add confidence branch
        self.fc_conf = nn.Linear(output_dim, 6)  # 4x4 matrix = 16 elements
    ...
    def forward(self, decout, img_shape):
        ...
        conf_raw = self.fc_conf(feat)  # [B,6]
        conf_raw = conf_raw.view(B, 6, 1)
        
        # Process confidence using sigmoid mode from postprocess.py
        mode, vmin, vmax = self.conf_mode
        if mode == 'sigmoid':
            conf = (vmax - vmin) * torch.sigmoid(conf_raw) + vmin
        elif mode == 'exp':
            conf = vmin + conf_raw.exp().clip(max=vmax-vmin)
        else:
