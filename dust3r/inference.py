...
def loss_of_one_batch(batch, model, criterion, device, symmetrize_batch=False, use_amp=False, ret=None, return_pose=False):
    view1, view2 = batch
    ...
    with torch.cuda.amp.autocast(enabled=bool(use_amp)):
        ...
        if return_pose:
            pred1, pred2, res_pose= model(view1, view2, return_pose=return_pose)
        else:
            pred1, pred2 = model(view1, view2)
        ...
        with torch.cuda.amp.autocast(enabled=False):
            if return_pose:
                loss = criterion(view1, view2, pred1, pred2, res_pose) if criterion is not None else None
            else:
...
def get_pred_pts3d_pose(gt, pred, pred_pose):
    if 'depth' in pred and 'pseudo_focal' in pred:
        ...
        pts3d = pred['pts3d_in_other_view']  # return!
    assert pred_pose is not None
    pts3d = geotrf(pred_pose, pts3d)
    return pts3d
...
def get_pred_pts3d(gt, pred, use_pose=False):
    ...
    if use_pose:
        ...
        print('camera_pose', camera_pose.shape)
